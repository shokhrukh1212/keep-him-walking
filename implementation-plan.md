...
- [x] Add unit/domain tests and Playwright smoke/regression coverage, including loading all seven complete reduced-motion country scenes. The provider-backed sponsor lifecycle and 70-minute canonical transition observation remain rehearsal gates below.
...
| `pnpm rehearse:phase2` 70-minute observation | Pending | The latest observer remained healthy for 30.1 minutes, then Vercel CLI timed out while refreshing its protection cookie. The runner now retains a healthy authenticated session when an explicit refresh command fails, but the interrupted run is not claimed as a pass. The staged functional proof does not replace the planned uninterrupted ten-minute-per-country observation. |
