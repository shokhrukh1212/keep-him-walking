...
The Phase 1 truthful shared-state slice and Phase 1.5 Tashkent continuous-world proof are the completed technical baseline. Phase 2 is implemented on `phase-2-seven-day-mvp` as a preview-only seven-country rehearsal build. Its migrations, external cultural review, production Rive delivery, provider checkout and physical-device/human gates remain deliberately open; see `implementation-plan.md` and `docs/phase-2-results.md` before enabling or scheduling it.
