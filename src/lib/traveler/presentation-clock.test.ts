...
describe("two-track presentation clock",()=>{
  it("does not restart the gait when fresh confirmations arrive",()=>{
    ...
    c.accept(runtime(20,40,10_000),50_000,10_000);
    const after=c.sample(10_020);
    expect(after.rawSeconds).toBeCloseTo(before.rawSeconds+0.02,2);
    expect(after.distanceMetres).toBeCloseTo(before.distanceMetres+0.03,2);
  ...
  it("never advances beyond a confirmed lease and can renew after reconnect",()=>{
    const c=new PresentationClock();c.accept(runtime(),50_000,0);
    expect(c.sample(51_000)).toEqual({rawSeconds:60,distanceMetres:100,traveling:false});
    expect(c.sample(90_000)).toEqual({rawSeconds:60,distanceMetres:100,traveling:false});
  ...
  it("caps both tracks at sixty seconds even when the lease is longer",()=>{
    ...
    expect(c.sample(70_000)).toEqual({rawSeconds:60,distanceMetres:90,traveling:false});
  });
  ...
  it.each([70_000, 90_000])("keeps the sixty-second guess independent of a %ims lease",(ttl)=>{
    ...
    expect(at(ttl)).toEqual({rawSeconds:60,distanceMetres:90,traveling:false});
  });
  it("renews an unchanged authoritative sample without rewinding its anchors",()=>{
    ...
    expect(c.sample(4_000)).toMatchObject({rawSeconds:14,distanceMetres:31,traveling:true});
    ...
    expect(renewed.distanceMetres).toBeGreaterThan(31);
  });
