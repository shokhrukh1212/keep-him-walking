...
const reviewedParis = countryPackV3Schema.parse({
  ...parisCountryPackV2,
  conversations: parisCountryPackV2.conversations.map((script) => ({ ...script, review: "approved" })),
});
...
describe("playedEncounters", () => {
  it("lists begun, uncancelled conversations newest first, with speaker and place", () => {
    ...
    const played = playedEncounters(reviewedParis, rows, 700);
  ...
  it("does not expose a pending script even if an old row already scheduled it", () => {
    expect(playedEncounters(parisCountryPackV2, [row(10, "paris-station-hello")], 100)).toEqual([]);
  });
});

describe("mergeEncounterLog", () => {
  it("keeps the newest few and returns the same log when nothing is new", () => {
    const first = playedEncounters(reviewedParis, [row(150, "paris-station-hello")], 200);
    const log = mergeEncounterLog([], first);
    expect(mergeEncounterLog(log, first)).toBe(log);

    const later = playedEncounters(reviewedParis, [
