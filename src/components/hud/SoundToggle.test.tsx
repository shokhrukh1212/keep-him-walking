...
describe("SoundToggle", () => {
  it("is one pressed-state button whose name says what is on", async () => {
    ...
    const off = screen.getByRole("button", { name: "Ambient sound off" });
    expect(off).toHaveAttribute("aria-pressed", "false");
    await userEvent.click(off);
    expect(onToggle).toHaveBeenCalledTimes(1);

    rerender(<SoundToggle enabled available resumesOnTap={false} onToggle={onToggle} />);
    expect(screen.getByRole("button", { name: "Ambient sound on" })).toHaveAttribute("aria-pressed", "true");

    rerender(<SoundToggle enabled={false} available resumesOnTap onToggle={onToggle} />);
    expect(screen.getByRole("button", { name: "Ambient sound off, resumes on your first tap" })).toBeInTheDocument();

    rerender(<SoundToggle enabled={false} available={false} resumesOnTap={false} onToggle={onToggle} />);
    expect(screen.getByRole("button", { name: "Ambient sound unavailable" })).toBeDisabled();
  });
});
