...
it("edits locally, keeps zone drafts separate, and copies the complete stage block", async () => {
  ...
  render(<StageCalibration zones={tbilisiCountryPackV1.route.zones} />);
  ...
  fireEvent.change(screen.getByRole("combobox"), {target: {value: "balcony-lanes"}});
  expect(JSON.parse(screen.getByTestId("stage-json").textContent!).stage).toEqual(tbilisiCountryPackV1.route.zones[1].stage);
  fireEvent.change(screen.getByRole("combobox"), {target: {value: "rustaveli-arrival"}});
