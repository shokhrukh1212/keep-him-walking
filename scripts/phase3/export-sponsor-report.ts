...
const { data, error } = await adminClient().from("sponsor_daily_metrics")
  .select("metric_date,sponsorship_id,impressions,engaged_views,cta_clicks,updated_at")
  .gte("metric_date", from).lte("metric_date", to).order("metric_date");
if (error) throw error;
process.stdout.write("metric_date,sponsorship_id,impressions,engaged_views,cta_clicks,updated_at\n");
for (const row of data ?? []) process.stdout.write([row.metric_date, row.sponsorship_id, row.impressions, row.engaged_views, row.cta_clicks, row.updated_at].join(",") + "\n");
