...
export function renderPackModule(slug: string, input: PackAuthoring, build?: PackBuild): string {
  ...
  const value = {
    ...
    sourceNotes: [
      ...
      "The owner supplied the text and artwork; the build pipeline made no cultural claims.",
    ],
    culturalReview: {
      reviewerName: null,
      reviewedAt: null,
    ...
    authoredAssets: { nightZoneIds, lightsZoneIds },
  };
...
export function renderPromptReadme(slug: string, input: PackAuthoring): string {
  ...
  lines.push(
    ...
    "## Build",
    "",
    "Run `pnpm pack:build " + slug + "`. It requires all five masters and the landmark night image. Optional `lights.png` files beside any master become dusk overlays.",
    "",
  );
  return `${lines.join("\n")}\n`;
}
...
async function processPanorama(source: string, target: string): Promise<{ palette: [string, string, string]; bytes: number }> {
  ...
  const output = await sharp(blended, { raw: info }).webp({ quality: 84, effort: 5 }).toBuffer();
  await writeAtomic(target, output);
  return { palette: dominantPalette(blended, info.channels), bytes: output.byteLength };
}
...
export async function buildPack(root: string, rawSlug: string, now = new Date()): Promise<PackBuild> {
  ...
  for (const kind of PACK_ZONE_KINDS) {
    ...
    const bytes = day.bytes + (nightBytes ?? 0) + (lightsBytes ?? 0);
    build.transferBytes += bytes;
    build.zones[id] = { palette: day.palette, night: nightBytes !== null, lights: lightsBytes !== null, bytes };
  }
