...
| Natural walking pace | Migration 0037 applied and remote-verified on dev: audience presence starts/stops the journey, but viewer count never changes speed. Migration 0039 (13 September) raises the one pace to 1.5 m/s so his feet stay planted |
...
| Image delivery | Hashed immutable renditions, current + next place only. Paris v3 passed 95/95 CDN checks. Brussels Day 2 now has 95 R2-only scene renditions (95/95 CDN checks), one postcard and eleven hosted source images; its visual acceptance remains with the owner. `ASSET_BASE_URL` is set in Vercel Production and Preview. Preview origins still need a CORS decision (D8) |
...
| Prelaunch waiting experience | Implemented on branch `prelaunch-traveler-preview` (14 September). With the launch switch off the page is an honest "Paris · Preview": he faces the viewer and says eight owner-written lines, the first about 5 s after he loads and then one every 180 visible seconds, each with a synchronized caption. Unit and emulated-browser verified; owner visual acceptance and physical phones pending |
| 16:00 UTC reconciliation | Implemented and remote database verified. Minute runs come from an external cron-job.org job (owner) with `CRON_SECRET`. `vercel.json` keeps a daily 16:00 UTC backup, which the free plan accepts (D5) |
