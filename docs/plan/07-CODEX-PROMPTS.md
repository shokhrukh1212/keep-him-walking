...
4. Convert one Phase 3 city (Sofia) with the new pipeline as the worked example; leave
   the others.
```

**What you should see:** `pnpm pack:new lisbon` creates a stub pack and a README of
image prompts; after dropping PNGs in, `pnpm pack:build lisbon` produces the webps and
a valid pack; Sofia now has five distinct zones ready for calibration.
