...
| Long canonical observation | Route/motion/event evidence pass: 4,040,358 ms uninterrupted, all seven cities in order, 505–612 observed seconds and 4–5 zones per city, 14 traveler states, encounter/action cadence, seven votes, sponsor disclosure/redirect, offline recovery, full motion and the automatic OS reduced-motion fallback. The run exposed the same-page postcard state defect after all those assertions passed. `docs/phase-2-canonical-rehearsal.json` records the exact evidence; the fix then passed a dedicated same-page regression and the guarded seven-country postcard rehearsal. |
