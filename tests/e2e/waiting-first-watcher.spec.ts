...
test("an arrival after the last lease expires sees the first-watcher beat and wake card", async ({ browser }, testInfo) => {
  ...
  await expect(pageB.getByRole("status", { name: /Walking rule: Waking up/ })).toBeVisible();
  ...
  await expect(pageB.getByLabel("You woke him up")).toContainText("Paris");
  await expect(pageB.getByLabel("You woke him up").getByRole("link", { name: "Share on X" }))
    .toBeVisible();
