...
- **Caption and framing.** `PreviewCaption` renders only in prelaunch, `aria-hidden`, with nothing
  ...
  - Wider than 600 px it is fixed to his left at chest height, from the published
    `--stage-bottom` and `--stage-person-height`, and kept above the footer.
  - At 600 px and below it is the footer's first row, three lines tall, present whether he
    speaks or not.
  - There, `stageLayout(…, { groundReservePx })` raises the ground so his soles stay 10 px above
    the footer's top (measured with a ResizeObserver, never per frame), but never leaves less
    than 20% of the screen above his head. A zero reserve, used everywhere outside the phone
    preview, returns exactly the previous layout. Pixi reads the reserve through its runtime
    ref, so the world is not rebuilt, and eases a change over 400 ms.
