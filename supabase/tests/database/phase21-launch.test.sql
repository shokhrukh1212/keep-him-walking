...
select has_function('public', 'record_presence_heartbeat_v12', array['uuid','text','text','text','boolean','timestamp with time zone','integer','numeric','real','integer','text'], 'the launch-guarded action-aware heartbeat exists');
select has_function('public', 'read_bootstrap_bundle_v14', array['text','timestamp with time zone','integer','numeric','integer','integer','real','numeric'], 'the action-aware launch bootstrap exists');
...
create temporary table launch_seed as select public.seed_season1_launch(
  jsonb_build_object(
    ...
    'day', jsonb_build_object(
      'dayNumber', 1, 'countryCode', 'GB', 'countryName', 'United Kingdom',
      'cityName', 'London', 'timeZone', 'Europe/London',
      'startsAt', '2034-09-20T16:00:00Z', 'endsAt', '2034-09-21T16:00:00Z',
      'scenePackId', 'london-v1', 'storySummary', 'The journey begins in London.',
...
select is((select scene_pack_id from public.country_days cd join public.journeys j on j.id = cd.journey_id where j.slug = 'keep-him-walking-season-1'), 'london-v1', 'Day 1 accepts the reviewed pack selected by the launch plan');
...
select is(
  public.seed_season1_launch((
    select jsonb_build_object(
      ...
      'day', jsonb_build_object('dayNumber', 1, 'countryCode', 'GB', 'countryName', 'United Kingdom', 'cityName', 'London', 'timeZone', 'Europe/London', 'startsAt', '2034-09-20T16:00:00Z', 'endsAt', '2034-09-21T16:00:00Z', 'scenePackId', 'london-v1', 'storySummary', 'The journey begins in London.', 'postcardBackgroundUrl', '/postcards/london/v1/background.webp'),
      'events', '[]'::jsonb,
...
select is(
  public.switch_country_day_pack(
    ...
    'london-v1', 'tashkent-v4', '2034-09-10T12:02:00Z'
  ),
  true,
...
select is(has_function_privilege('anon', 'public.record_presence_heartbeat_v12(uuid,text,text,text,boolean,timestamptz,integer,numeric,real,integer,text)', 'EXECUTE'), false, 'anon cannot call the guarded heartbeat');
select is(has_function_privilege('service_role', 'public.read_bootstrap_bundle_v14(text,timestamptz,integer,numeric,integer,integer,real,numeric)', 'EXECUTE'), true, 'service role can read the launch bundle');
select is(
  (public.read_bootstrap_bundle_v14(repeat('c', 64), '2034-09-20T15:00:00Z', 50, 1.8, 100, 60, 5, 30) -> 'bundle')::text,
  'null',
