-- Corrects 0029: launch identity is content data, not a Tashkent database rule.
-- The CLI validates the requested pack against the reviewed registry before it
-- calls this RPC; this function validates the complete immutable plan.
...
declare
  ...
  v_pack_id text := nullif(trim(p_plan #>> '{day,scenePackId}'), '');
...
begin
  if v_slug <> 'keep-him-walking-season-1'
    ...
    or v_pack_id is null
    or p_plan #>> '{day,countryCode}' !~ '^[A-Z]{2}$'
    or nullif(trim(p_plan #>> '{day,countryName}'), '') is null
    or nullif(trim(p_plan #>> '{day,cityName}'), '') is null
    or nullif(trim(p_plan #>> '{day,timeZone}'), '') is null
    or nullif(trim(p_plan #>> '{day,postcardBackgroundUrl}'), '') is null
    ...
    if v_journey.launch_at is distinct from v_launch_at
      ...
      or not exists (
        ...
        where cd.journey_id = v_journey.id and cd.day_number = 1
          and cd.scene_pack_id = v_pack_id and cd.starts_at = v_launch_at
      )
  ...
  insert into public.country_days (
    ...
    postcard_background_url, arrival_mode, updated_at
  ) values (
    ...
    v_pack_id, 'scheduled', p_plan #>> '{day,storySummary}',
    p_plan #>> '{day,postcardBackgroundUrl}', 'walk', p_now
  ...
  return jsonb_build_object(
    ...
    'scenePackId', v_pack_id
  );
end;
$$;

comment on function public.seed_season1_launch(jsonb, timestamptz) is
  'Creates an immutable Season 1 launch from a CLI-validated reviewed pack.';
